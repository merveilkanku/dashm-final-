...
function App() {
  ...
  useEffect(() => {
    ...
    const setupDeepLinks = () => {
      CapApp.addListener('appUrlOpen', async (data) => {
        ...
        // Supabase OAuth callback handling for native deep links
        if (url.host === 'callback' || url.host === 'login-callback' || url.pathname.includes('callback')) {
          ...
          if (accessToken && refreshToken) {
            ...
            if (error) {
              ...
              toast.error("Erreur d'authentification native");
            } else {
    ...
    const initSession = async () => {
        try {
            // 1. Handle OAuth popup callback if we are in a popup (Web)
  ...
  const fetchRestaurants = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log("DB vide ou null, chargement Mocks...");
  ...
  const renderContent = () => {
      ...
      if (showAuth) {
        return (
          <>
            ...
            <AuthScreen onLogin={handleManualLogin} isSupabaseReachable={isSupabaseReachable} onBackToGuest={() => setShowAuth(false)} />
          </>
        );
      }
    ...
    // 3. Logged in as Business or Staff
