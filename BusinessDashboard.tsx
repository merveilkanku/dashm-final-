...
import { Restaurant, MenuItem, User, UserRole, Order, OrderStatus, Promotion, Theme, Language, AppFont } from './types';
import { 
  ...
  Store, ExternalLink, Info, HelpCircle, Book, Mail, Navigation
} from 'lucide-react';
...
type DashboardView = 'overview' | 'orders' | 'menu' | 'sales' | 'settings' | 'marketing' | 'marketplace' | 'subscribers' | 'team' | 'reviews';
...
export const BusinessDashboard: React.FC<Props> = ({ user, restaurant, onUpdateRestaurant, onUpdateUser, onLogout, theme, setTheme, language, setLanguage, font, setFont }) => {
  ...
  const [settingsSubView, setSettingsSubView] = useState<'menu' | 'verification' | 'content' | 'privacy' | 'delivery' | 'hours' | 'notifications'>('menu');
  ...
  const renderOverview = () => {
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="flex flex-col md:flex-row gap-6">
              <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex-1">
                  <h3 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center">
                      <Bike size={18} className="mr-2 text-brand-600"/> Livreurs à proximité
                  </h3>
                  <div className="space-y-3">
                      {deliveryPersonnel.filter(p => p.deliveryInfo?.isAvailable).slice(0, 5).map(p => (
                          <div key={p.id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                              <div className="flex items-center space-x-3">
                                  <div className="w-8 h-8 bg-brand-100 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400 rounded-full flex items-center justify-center font-bold text-xs">
                                      {p.name.charAt(0)}
                                  </div>
                                  <div>
                                      <p className="text-sm font-bold text-gray-700 dark:text-gray-200">{p.name}</p>
                                      <p className="text-[10px] text-gray-400 uppercase font-bold">{p.deliveryInfo?.vehicleType || 'Moto'}</p>
                                  </div>
                              </div>
                              {p.phoneNumber && (
                                  <a href={`tel:${p.phoneNumber}`} className="p-2 bg-white dark:bg-gray-800 text-brand-600 rounded-lg shadow-sm hover:bg-brand-50 transition-colors">
                                      <Phone size={14} />
                                  </a>
                              )}
                          </div>
                      ))}
                      {deliveryPersonnel.filter(p => p.deliveryInfo?.isAvailable).length === 0 && (
                          <p className="text-gray-400 text-center text-sm py-4 italic">Aucun livreur disponible pour le moment.</p>
                      )}
                  </div>
                  <button onClick={() => navigateTo('team')} className="w-full mt-4 text-brand-600 dark:text-brand-400 text-sm font-bold hover:underline">
                ...
                <div key={review.id} className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <h4 className="font-bold text-gray-900 dark:text-white">{review.profiles?.full_name || 'Utilisateur'}</h4>
    ...
    if (settingsSubView === 'menu') {
        return (
            <div className="space-y-4 max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                >
                    <div className="flex items-center">
                        <div className="bg-green-100 dark:bg-green-900/30 p-3 rounded-xl mr-4 text-green-600 dark:text-green-400 group-hover:scale-110 transition-transform">
                            <Truck size={24} />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 dark:text-white">Livraison et Zones</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Frais, rayons et temps de livraison</p>
                        </div>
                    </div>
                    <ChevronRight size={20} className="text-gray-400" />
                </button>
                ...
                >
                    <div className="flex items-center">
                        <div className="bg-yellow-100 dark:bg-yellow-900/30 p-3 rounded-xl mr-4 text-yellow-600 dark:text-yellow-400 group-hover:scale-110 transition-transform">
                            <Clock size={24} />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 dark:text-white">Horaires d'ouverture</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Gérez vos horaires par jour</p>
                        </div>
                    </div>
                    <ChevronRight size={20} className="text-gray-400" />
                </button>
                ...
                >
                    <div className="flex items-center">
                        <div className="bg-pink-100 dark:bg-pink-900/30 p-3 rounded-xl mr-4 text-pink-600 dark:text-pink-400 group-hover:scale-110 transition-transform">
                            <Bell size={24} />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 dark:text-white">Notifications</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Alertes commandes et marketing</p>
                        </div>
                    </div>
                    <ChevronRight size={20} className="text-gray-400" />
                </button>
                ...
                >
                    <div className="flex items-center">
                        <div className="bg-brand-100 dark:bg-brand-900/30 p-3 rounded-xl mr-4 text-brand-600 dark:text-brand-400 group-hover:scale-110 transition-transform">
                            <HelpCircle size={24} />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 dark:text-white">Centre d'aide & Support</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Guides, FAQ et contact support</p>
                        </div>
                    </div>
                    <ChevronRight size={20} className="text-gray-400" />
                </button>
  ...
  const renderSalesAndInventory = () => {
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
            ...
            <div className="flex items-center space-x-2 bg-gray-50 dark:bg-gray-700 p-1.5 rounded-lg border border-gray-200 dark:border-gray-600">
              <input 
                ...
                className="bg-transparent text-sm text-gray-700 dark:text-gray-200 outline-none border-none p-1"
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
             <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl">
                 <p className="text-sm font-bold text-gray-500 dark:text-gray-400">
                   {salesStartDate || salesEndDate ? 'Période Sélectionnée' : 'Total Global'}
                 </p>
                 <p className="text-2xl font-black text-gray-900 dark:text-white mt-1">
                   ${(salesStartDate || salesEndDate ? filteredRevenue : revenue).toFixed(2)}
                 </p>
             </div>
             <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl">
                 <p className="text-sm font-bold text-gray-500 dark:text-gray-400">
                   {salesStartDate || salesEndDate ? 'Total Global' : 'Ce Mois'}
                 </p>
                 <p className="text-2xl font-black text-brand-600 dark:text-brand-400 mt-1">
                   ${(salesStartDate || salesEndDate ? revenue : monthlyRevenue).toFixed(2)}
                 </p>
             </div>
             <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mt-6">
        <h3 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center">
          <Receipt size={20} className="mr-2 text-brand-600"/> Historique des Commandes (Période Sélectionnée)
        </h3>
        <div className="overflow-x-auto">
                ...
                <tr key={order.id} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      ...
                      {order.status === 'completed' ? 'Terminée' : order.status === 'cancelled' ? 'Annulée' : order.status}
                    </span>
                  </td>
                </tr>
              )) : (
                    ...
                    <div>
                        ...
                        <div className="mb-3">
                             <button
                                ...
                                onClick={async () => {
                                    if (Capacitor.isNativePlatform()) {
                                        const file = newPromoType === 'video'
                                            ? await pickFile(['video/*'])
                                            : await pickImage();
                                        if (file) {
                                            setPromoFile(file);
                                            setNewPromoUrl('');
