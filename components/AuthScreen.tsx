...
export const AuthScreen: React.FC<Props> = ({ onLogin, isSupabaseReachable = true, onBackToGuest }) => {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      const currentOrigin = isNative ? 'com.dashmeals.android://callback' : window.location.origin;
      console.log("OAuth Redirect URL:", currentOrigin);

      // Detect if we are in the AI Studio preview
      const isPreview = !isNative && window.location.origin.includes('.run.app');

      if (isPreview || isNative) {
          // In preview (iframe) or Native, we use Capacitor Browser or popup
          ...
          if (data?.url) {
            if (isNative) {
              // Open using Capacitor Browser plugin
              await Browser.open({ url: data.url, windowName: '_self' });
            } else {
              // Open popup for web preview
              const width = 500;
              const height = 650;
  ...
  return (
    <div className="min-h-screen bg-brand-50 flex flex-col items-center justify-center p-4 relative">
      {onBackToGuest && (
        <button 
          onClick={onBackToGuest}
          className="absolute top-4 left-4 p-2 bg-white rounded-full shadow-md text-gray-600 hover:text-brand-600 transition-colors z-10"
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden">
        ...
        {!isStaffMode && (
          <div className="flex border-b">
            ...
            <button 
              ...
              className={`flex-1 py-4 text-sm font-bold flex items-center justify-center relative ${role === 'delivery' ? 'text-brand-600 border-b-2 border-brand-600 bg-brand-50' : 'text-gray-500'}`}
            >
              <Bike size={18} className="mr-2" />
              Livreur
              <span className="absolute top-1 right-1 bg-green-500 text-white text-[8px] px-1 rounded-full animate-pulse">NOUVEAU</span>
            </button>
          </div>
        )}
        ...
        <div className="p-6 pb-2">
            <h2 className="text-xl font-bold text-gray-800 text-center mb-2">
                {isStaffMode ? 'Connexion Équipe' : (isLogin ? 'Connexion' : 'Créer un compte')} {role === 'business' && !isStaffMode ? 'Partenaire' : ''}
            </h2>
            {!isLogin && !isStaffMode && (
            ...
            {isLogin && !isStaffMode && <div className="mb-6"></div>}
        ...
        {isStaffMode ? (
          <form onSubmit={handleStaffLogin} className="px-6 pb-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Nom de l'établissement</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Votre Nom</label>
              <div className="relative">
            ...
            >
              {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Se connecter à l\'équipe'}
            </button>
        ...
        <div className="bg-gray-50 p-4 text-center border-t space-y-2">
          <button 
            ...
            className="text-sm text-brand-600 font-bold hover:underline block w-full"
          >
          ...
          {isLogin && !isStaffMode && (
            <button 
              ...
              className="text-xs text-gray-500 hover:text-brand-600 font-medium flex items-center justify-center w-full"
            >
              <KeyRound size={12} className="mr-1" /> Connexion Équipe (PIN)
            </button>
          )}
        </div>
      </div>
