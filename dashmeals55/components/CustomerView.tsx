...
export const CustomerView: React.FC<Props> = ({ user, allRestaurants, onLogout, theme, setTheme, language, setLanguage, font, setFont }) => {
  ...
  // Get list of verified restaurants with promotions (Network Ads)
  const verifiedNetworkAds = allRestaurants.filter(r => r.isVerified && promotionsMap[r.id] && promotionsMap[r.id].length > 0);
      ...
      <main className="p-4 pt-2">
        ...
        {viewMode === 'list' || viewMode === 'map' ? (
            <>
                ...
                {/* NETWORK ADS (Verified Restaurants) - Hidden during search */}
                {!searchQuery && verifiedNetworkAds.length > 0 && (
                    <div className="mb-6 -mx-4 px-4 overflow-x-auto no-scrollbar">
                        <div className="flex items-center mb-2">
                            <Star className="text-yellow-500 mr-1.5" size={14} fill="currentColor" />
                            <span className="text-xs font-bold text-gray-800 dark:text-gray-200 uppercase tracking-wider">À la Une</span>
                        </div>
                        <div className="flex space-x-4">
                            {verifiedNetworkAds.map(r => (
                                <button 
                                    ...
                                    className="flex flex-col items-center space-y-1 min-w-[72px] group"
                                >
                                    <div className="w-18 h-18 rounded-full p-[3px] bg-gradient-to-tr from-blue-500 to-purple-500 shadow-md group-hover:scale-105 transition-transform">
                                        <div className="w-full h-full rounded-full border-2 border-white dark:border-gray-800 overflow-hidden relative">
                                    ...
                                    <span className="text-[11px] font-bold text-gray-800 dark:text-gray-200 truncate w-full text-center">{r.name}</span>
                                </button>
                            ))}
                        </div>
                    </div>
