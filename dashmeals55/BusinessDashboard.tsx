  ...
  const submitVerificationStep1 = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (idCardFile) {
            ...
            // Utiliser le bucket 'images' au lieu de 'documents'
...
const uploadVerificationDocument = async (file: File, type: 'id_card' | 'business_license'): Promise<string | null> => {
    try {
        ...
        // Générer un nom unique avec l'ID du restaurant dans un dossier spécifique
        ...
        // Upload dans le bucket 'images' (qui fonctionne déjà)
...
const uploadImage = async (fileOrBlob: File | Blob, bucket: string = 'images'): Promise<string | null> => {
    try {
        ...
        // Si c'est un document, accepter les PDF aussi
        if (file.type === 'application/pdf') {
            maxSize = 10 * 1024 * 1024; // 10MB pour les PDF
        ...
        // Mettre les documents de vérification dans un dossier spécifique
  ...
  const addItem = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Si pas d'image uploadée et pas d'image existante, utiliser une image par défaut
        if (!imageUrl) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                onChange={e => setSettingsForm({ ...settingsForm, latitude: parseFloat(e.target.value)  || 0  })}
              />
            </div>
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 flex items-center">
              ...
              <input
                ...
                onChange={e => setSettingsForm({ ...settingsForm, longitude: parseFloat(e.target.value)  || 0  })}
              />
            </div>
          </div>
